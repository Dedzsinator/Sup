defmodule Sup.Messaging.RichMediaService do
  ...
  def get_media_stats(user_id) do
    ...
    total_size = from(ma in MediaAttachment,
      where: ma.uploaded_by == ^user_id,
      select: sum(ma.file_size)
    ) |> Repo.one() || 0

    file_count = from(ma in MediaAttachment,
      where: ma.uploaded_by == ^user_id,
      select: count(ma.id)
    ) |> Repo.one()

    by_type = from(ma in MediaAttachment,
      where: ma.uploaded_by == ^user_id,
    ...
    %{
      ...
      by_type: Enum.map(by_type, fn {type, count, size} ->
  ...
  defp validate_file_content(file_data, file_type) do
    ...
    case {file_type, signature} do
      {"image", <<0xFF, 0xD8, 0xFF, _::binary>>} -> :ok  # JPEG
      {"image", <<0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A>>} -> :ok  # PNG
      {"image", <<"GIF8", _::binary>>} -> :ok  # GIF
      {"image", <<"RIFF", _::binary-size(4), "WEBP">>} -> :ok  # WebP
      {_, _} -> :ok  # For now, allow other types without strict validation
    end
  end
  ...
  defp delete_physical_file(file_path) do
    case File.rm(file_path) do
      ...
      {:error, reason} -> 
        ...
        :ok  # Don't fail the operation if file deletion fails
    end
  end
end
