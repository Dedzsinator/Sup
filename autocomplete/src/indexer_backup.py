...
class VectorIndexer:
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            stats.update({
                ...
                'faiss_total': self.index.ntotal if hasattr(self.index, 'ntotal') else 0
            })
            return stats
    ...
    def load(self, index_path: Union[str, Path], metadata_path: Optional[Union[str, Path]] = None):
        ...
        with self._lock:
            ...
            if self.config.use_gpu and faiss.get_num_gpus() > 0:
                gpu_resource = faiss.StandardGpuResources()
                self.index = faiss.index_cpu_to_gpu(gpu_resource, self.config.gpu_device, cpu_index)
            else:
...
class HybridIndexer:
    ...
    def search_all(
        ...
        for name, indexer in self.indexers.items():
            ...
            all_results.extend(zip(texts, similarities, metadata))
        ...
        if top_results:
            texts, similarities, metadata = zip(*top_results)
