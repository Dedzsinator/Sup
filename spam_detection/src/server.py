...
async def submit_feedback(
    ...
    try:
        ...
        logger.info(f"Feedback stored: '{message_text[:50]}...' -> spam={is_spam} (user: {user_id})")
        ...
        if ENHANCED_AVAILABLE and spam_stats["feedback_count"] % 100 == 0:
            logger.info("Sufficient feedback collected for potential model update")
        
        return {
            "status": "feedback_received",
            "message": "Thank you for your feedback. It will be used to improve the model.",
            "feedback_id": f"fb_{int(time.time())}_{spam_stats['feedback_count']}",
